...
2. 共通コンポーネント化
   - `packages/web/app` → `packages/web-core`：UI・ミドルウェアの共通化
     - ただし`server.ts`は`web`（`packages/wrangler`）に残存
   - データアクセス層の抽象化実装
3. 起動コマンド・スクリプト設定の調整
   - ルートpackage.jsonの更新
   - Docker Compose設定の調整
...
- packages/wranglerでのwrangler起動
- リファクタリング後のwrangler環境でのE2E動作確認（18/21テスト成功維持）
- ローカル・Docker両環境での動作確認

### フェーズ6: Node.js + オンメモリSQLite実装 - 現状HonoXで動かず：断念

**実装内容**:

1. 新規パッケージ作成：wrangler, wrangler-e2eのコピペで内部も適切に書き換える。
...
4. DBをオンメモリにしてサーバーが立ち上がって操作できるか確認：ユーザーに依頼する
5. ローカルサーバー起動からローカルサーバーに向けたE2Eが動くか確認：マイグレーションがいつどこでどう起動するか？
6. (Dockerなしで)ローカルサーバーが独立したオンメモリDBで動かせるか確認
7. E2Eテストではテスト時にbeforeEachなどで独立したサーバーをキックしてE2Eできるか確認
8. テストでエラーが起こったときに即止まるように対処
...
### フェーズ7: Node.js + DynamoDB実装 - 断念

前提としてHonoXがうまくつかえるかがキーだったが、前段でHonoX+（統合テスト・E2Eテスト用のオンメモリSQLite）が使えなくなったためこれは対応断念。

**要注意**：原則としてオンメモリSQLiteと同じように対処したい。後でもう少し調整が必要。統合テストにアクセスするデータベース名など。

**実装内容想定**:

1. 新規パッケージ作成：node-onmemsqlite, node-onmemsqlite-e2eのコピペで内部も適切に書き換える。
...
- **現在の状況**: 18/21テスト成功（削除テスト×3が失敗）
- **問題**: データベースリセット機能が完全に動作していない
  - 各テスト実行後にデータが蓄積される（11件まで増加を確認）
  - reset.sqlスクリプト自体は正しく、手動実行もエラーなし
  - wranglerローカルデータベースの永続化メカニズムに問題の可能性
- **調査項目**:
  - wranglerローカルデータベースの永続化メカニズム調査
  - Dockerとローカル環境でのwrangler動作差異の解明
  - テスト間データベース分離の代替手段検討
- **影響**: 基本的なCRUD動作は確認済み、削除機能のテストのみ影響
