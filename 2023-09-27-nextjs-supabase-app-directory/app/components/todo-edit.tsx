...
export default function EditTask() {
  ...
  async function submitHandler(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("todos").update({ title: editedTask.title }).eq("id", editedTask.id);
    reset();
  ...
  return (
    <div className="m-5 text-center">
      ...
      <form onSubmit={submitHandler}>
        ...
        <button
          type="submit"
          className="my-2 rounded border border-gray-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700"
        >{editedTask.id === "" ? "Create" : "Update"}</button>
      </form>
    </div>
  );
}
