 ...
 * @returns 全ベンチマーク結果
 */
export async function executeBenchmark(baseUrl: string, iterations: number): Promise<BenchmarkResult[]> {
 ...
 * @returns CSV形式の文字列
 */
export function generateCsvContent(results: readonly BenchmarkResult[]): string {
 ...
 * @returns 統計情報
 */
function calculateStats(results: readonly BenchmarkResult[], endpoint: string): EndpointStats {
	...
	const variance = responseTimes.reduce((sum, time) => sum + (time - avg) ** 2, 0) / responseTimes.length;
 ...
 */
export function generateMarkdownContent(results: readonly BenchmarkResult[]): string {
 ...
 */
export async function generateReport(results: readonly BenchmarkResult[], outputDir: string): Promise<string> {
