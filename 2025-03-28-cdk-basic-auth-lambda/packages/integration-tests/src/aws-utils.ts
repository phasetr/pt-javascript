 ...
 */
export async function getApiUrlFromCloudFormation(stackName?: string): Promise<string> {
  // 環境に応じたスタック名を生成
  ...
  try {
    ...
    throw new Error(`Failed to get API Gateway URL from CloudFormation stack: ${cfnStackName}`);
  } catch (error) {
...
export async function getLambdaUrl(functionName?: string): Promise<string> {
  ...
  try {
    ...
    if (response.Configuration?.FunctionArn) {
      ...
      // Lambda関数の情報からAPI GatewayのURLを推測することは難しいため、
      // 環境変数のURLを使用します
      const envVarName = `${environment.toUpperCase()}_API_URL`;
      const apiUrl = process.env[envVarName];
      
      if (apiUrl) {
        console.log(`Using API URL from environment variable ${envVarName}: ${apiUrl}`);
        return apiUrl;
      }
      
      // 環境変数が設定されていない場合は、デフォルトのURLを使用します
      // dev環境の場合は、既知のURLを使用
      if (environment === 'dev') {
        const defaultUrl = 'https://tmpi104l8f.execute-api.ap-northeast-1.amazonaws.com/prod/';
        console.log(`Using default API URL for dev environment: ${defaultUrl}`);
        return defaultUrl;
      }
      
      // 環境変数が設定されていない場合は、エラーをスローします
      throw new Error(`Environment variable ${envVarName} is not set. Please set it to the API Gateway URL.`);
    }
    
    // Lambda関数の情報が取得できない場合はエラー
    throw new Error(`Failed to get Lambda URL for function: ${lambdaFunctionName}`);
  } catch (error) {
...
export async function getApiUrl(): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      console.warn('Failed to get API URL from CloudFormation, falling back to Lambda function:', cfnError);
